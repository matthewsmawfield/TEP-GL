...
def load_citation_metadata():
    ...
    if not citation_file.exists():
        ...
        return {'version': 'v0.7', 'codename': 'Tortola', 'title': 'TEP-GL'}
    ...
    try:
        ...
        # Parse version string like 'v0.4 (Tortola)'
        pattern = r'^(v?[\d.]+)(?:\s*\(([^)]+)\))?$'
        match = re.match(pattern, version_str.strip())
        
        if match:
            version = match.group(1).lstrip('v')
            codename = match.group(2) or 'Tortola'
        else:
...
async def generate_pdf(quality: str = 'high', wait_time: float = 5.0, skip_build: bool = False):
    ...
    async with HTMLToPDFConverter() as converter:
        ...
        # Process with metadata (compresses in-place)
        process_pdf_with_metadata(final_pdf)
        
        # Copy compressed version to root directory
